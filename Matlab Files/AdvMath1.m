    ...
    case'Forward'
        ...
        while err_E_max(k)>=tolerance || err_E_min(k)>=tolerance
          for i=2:NodesL
            for j=2:NodesH
                if i<=NodesL/2
                    T(i,j,k+1) = T(i,j,k)+dt1*Alpha1*(((T(i,j,k)-2*T(i+1,j,k)+T(i+2,j,k))/dx^2)+((T(i,j+2,k)-2*T(i,j+1,k)+T(i,j,k))/dy^2));
                else
                    T(i,j,k+1) = T(i,j,k)+dt2*Alpha2*(((T(i,j,k)-2*T(i+1,j,k)+T(i+2,j,k))/dx^2)+((T(i,j+2,k)-2*T(i,j+1,k)+T(i,j,k))/dy^2));
                end
                % enforce Neumann boundary conditions insulation
%                 T(i,end,k+1) = T(i,end-1,k);
%                 T(i,1,k+1) =T(i,2,k);
%                 T(end,j,k+1) = T(end-1,j,k);
%                 T(1,j,k+1) = T(2,j,k);
          ...
          if round(err_E_min(k),5)==round(err_E_min(k-1),5) && err_E_min(k)~= 0 || k>5     % break out of loop
            break
          end
        end
