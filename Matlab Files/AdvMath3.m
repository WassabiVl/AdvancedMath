...
tolerance = 10;                   % tolerance for numerical simulation (0.5 deg Celsius)
...
Solution_type=questdlg('Which method you want to solve the time derivative with ?','Question','Backward','Backward');                     % solve with 2nd order Runge Kutte in time or 2 to solve with Euler
...
%% 3- Generate implicit matrix for 2D heat equation with Dirichlet in bottom and right and Neumann in top and left
   ...
   %while err_SS_max(k)>=tolerance_ss || err_SS_min(k)>=tolerance_ss 
    ...
    for i=2:NodesL                                                    % looping
        for j=2:NodesH
            ...
            if i == 2 && j== 2 % Bottom left corner (Dirichlet down and left)
                Tss2(i,j) = 1/sigma +4;
                Tss2(i,j+1) = -1;
                Tss2(i,j+NodesL-2) = -1;
            elseif i == NodesL-2 && j == 2 % Bottom right corner (Dirichlet down, Neumann right)
                Tss2(i,j) = 1/sigma +3;
                Tss2(i,j+1) = -1;
                Tss2(i,j+NodesL-2) = -1;
            elseif i ==1 && j == NodesH-2 % Bottom right corner (Dirichlet down, Neumann right)
                Tss2(i,j) = 1/sigma + 3;
                Tss2(i,j+1) = -1;
                Tss2(i,j+NodesL-2) = -1;
            elseif i == NodesL-2 && j == NodesH-2 % Bottom right corner (Dirichlet down, Neumann right)
                Tss2(i,j) = 1/sigma + 2;
                Tss2(i,j+1) = -1;
                Tss2(i,j+NodesL-2) = -1;
            end
...
switch Solution_type         
    case'Backward'
        ...
        err_E_max(k)=1000;                            % initial error
        err_E_min(k)=1000;                            % initial error
        while err_E_max(k)>=tolerance || err_E_min(k)>=tolerance
          for i=2:NodesL
            for j=2:NodesH
                 if i<=NodesL/2
                    T(i,j,k+1) = T(i,j,k) + Alpha1*( dt1/(dx^2) * ( T(i+2,j,k) - 2*T(i+1,j,k) + T(i,j,k)) + dt1/(dy^2) * ( T(i,j+2,k) - 2*T(i,j+1,k) + T(i,j,k))  ) ;
                 else
                    T(i,j,k+1) = T(i,j,k) + Alpha2*( dt2/(dx^2) * ( T(i+2,j,k) - 2*T(i+1,j,k) + T(i,j,k)) + dt2/(dy^2) * ( T(i,j+2,k) - 2*T(i,j+1,k) + T(i,j,k))  ) ;
                 end
                 % enforce Neumann boundary conditions insulation
                T(i,end) = T(i,end-1);
                T(end,j) = T(end-1,j);
                % if temp in middle is reached the min of the 4 inital temp
                % break
                if T(mid_i,mid_j) >= T_min
                    break;
                end
            end
          end
